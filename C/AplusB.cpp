...
//    for (i=99996; i<=100006; ++i) printf("%d ", T[i]);

    // T[i] contains the number of ways two a can sum up to i+50000
    // Add all of it together, then subtract when not pairwise distinct
    for (i=50000; i<= 150000; ++i) counter += T[i]*Q[i-50000].real(); 

    // When i=j, Q[ai] ways to sum up to 2*ai. Remove them
    for (i=25000; i<= 75000; ++i) counter -= Q[i].real()*Q[2*i-50000].real();

    // When (i OR j) = k, => (aj or ai) = 0, Q[0]*Q[ai] (*2 ways, for i and j)
    for (i=50000; i<= 150000; ++i) counter -= 2*Q[50000].real()*Q[i-50000].real();

    // Add back when i=j=k => 2*Q[0] ways
    counter += 2*Q[50000].real();
