/* Mistakes were made in this code */
...
public class KingOfTheNorth{
    ...
    public void run() throws IOException{
        ...
        int row = Integer.parseInt(dlist[0]);
        int col = Integer.parseInt(dlist[1]);

        grid = new int[row][col];

        for (int i = 0; i < row; ++i){
            dlist = r.readLine().split("\\s+");

            for (int j = 0; j < col; ++j){
                grid[i][j] = Integer.parseInt(dlist[j]);
            }
        }

        // Scan in the source
        dlist = r.readLine().split(" ");
        int sourceRow = Integer.parseInt(dlist[0]);
        int sourceCol = Integer.parseInt(dlist[1]);
        int source = 2 * (sourceRow * col + sourceCol);

        // Note that each vertex is now split into 2:
        // An incoming vertex, and an outgoing vertex
        // Also note that we need one more node to act as the sink.
        // Hence we need row*col*2 + 1 vertices in total
        DinicSolver dinic = new DinicSolver(row*col*2 + 1);
        int sink = row*col*2;
        ...
        for (int i = 0; i < row; ++i){
            for (int j = 0; j < col; ++j){
                ...
                for (int k = 0; k < 4; ++k){
                    ...
                    int newIncoming = (newrow * col + newcol) * 2;
                    dinic.addEdge(currIncoming + 1, newIncoming, INF);
                }
            }
        }

        // Add the connections to the sink vertex
        for (int i = 0; i < row; ++i){
            dinic.addEdge(2 * (i*col) + 1, sink, INF);
            dinic.addEdge(2 * (i*col + col - 1) + 1, sink, INF);
        }

        for (int j = 1; j < col-1; ++j){
            dinic.addEdge(2*j + 1, sink, INF);
            dinic.addEdge(2*((row-1) * col + j) + 1, sink, INF);
        }
    ...
    public static void main(String[] args) throws IOException{
        KingOfTheNorth solution = new KingOfTheNorth();
        solution.run();
    }
}
